...
export default function CalendlyEmbed({ contactPage }) {
  ...
  if (calendlyUrl) {
    return (
      <div className="overflow-hidden rounded-2xl border border-navy/10 shadow-card">
        <iframe
          src={calendlyUrl}
          title={contactPage.demoPlaceholder.iframeTitle}
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-navy/20 bg-pale-green/40 p-12 text-center">
      ...
      <h3 className="mt-5 text-xl font-bold text-dark-green">{contactPage.demoPlaceholder.heading}</h3>
      <p className="mt-2 max-w-sm text-navy/70">{contactPage.demoPlaceholder.description}</p>
      <Button href={`mailto:${siteConfig.email}`} variant="secondary" size="md" className="mt-6">
        {contactPage.demoPlaceholder.cta}
      </Button>
    </div>
  );
}
