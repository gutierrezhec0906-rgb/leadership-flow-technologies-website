...
export default function ContactInfo({ contactPage, demoHref }) {
  return (
    <div className="flex h-full flex-col justify-between rounded-2xl bg-dark-green p-8 text-off-white sm:p-10">
      <div>
        <h3 className="text-2xl font-bold">{contactPage.info.heading}</h3>
        <p className="mt-3 leading-relaxed text-off-white/80">{contactPage.info.description}</p>
      ...
      <Button href={demoHref} variant="secondary-dark" size="lg" className="mt-10 w-full">
        {contactPage.info.bookDemo}
      </Button>
    </div>
  );
}
